...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    s = [i for i in paragraphs if select(i) == True]
    if k <= len(s) - 1:
        return s[k]
    else:
        return ''
...
def accuracy(typed, source):
    ...
    if len(source_words) == 0 and len(typed_words) == 0:
        return 100.0
    elif (len(typed_words) == 0 and len(source_words) != 0) or (len(source_words) == 0 and len(typed_words) != 0) :
        return 0.0
    else:
        count = 0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        l = [diff_function(typed_word, word, limit) for word in word_list]
        if min(l) > limit:
            return typed_word
        else:
            return word_list[l.index(min(l))]
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:
        return 0
    if start == "" or goal == "":  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, times_per_player):
    ...
    for t in times_per_player:
        time.append([t[i] - t[i-1] for i in range(1, len(t))])
    ...
    for j in word_indices:
        ...
        for p in player_indices:
            if time(match, p, j) < t:
                t = time(match, p, j)
                player = p
        f[player].append(get_word(match, j))
    return f
    # END PROBLEM 10
